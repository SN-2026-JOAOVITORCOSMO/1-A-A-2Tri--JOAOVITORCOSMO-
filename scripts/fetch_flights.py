...
# (ocorre quando a API retorna o mesmo voo duas vezes no mesmo lote)
_seen: set = set()
_uniq: list = []
for _r in registros:
    _key = (
        _r["data_referencia"], _r["icao_empresa"],
        _r["numero_voo"], _r["icao_origem"],
        _r["icao_destino"], _r["etapa"],
    )
    if _key not in _seen:
        _seen.add(_key)
        _uniq.append(_r)
if len(_uniq) < len(registros):
    print(f"  {len(registros) - len(_uniq)} duplicata(s) removida(s) do lote.")
registros = _uniq
